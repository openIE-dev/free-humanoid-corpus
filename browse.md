...
Search across all 106 entries by name, prior-art notes, creator, or disclosed subsystem. Filters compose. Click any entry to expand its full record.
...
<div class="filters">
  ...
  <label>Year ≥
    <input id="filter-year-min" type="number" placeholder="1920" min="1900" max="2100">
  </label>
  <label>Year ≤
    <input id="filter-year-max" type="number" placeholder="2025" min="1900" max="2100">
  </label>
  <label>Subsystem tag (substring)
    <input id="filter-tag" placeholder="harmonic, zmp, rl, …" autocomplete="off">
  </label>
</div>
...
(async function () {
  ...
  function renderEntry(e) {
    ...
    row("morphology", e.morphology_family);
    row("dof", e.dof_count);
    row("actuator", e.actuator_type);
    row("height m", e.height_m);
    row("mass kg", e.mass_kg);
    row("payload kg", e.payload_kg);
    row("power", e.power_source);
    row("end-effector", e.end_effector);
